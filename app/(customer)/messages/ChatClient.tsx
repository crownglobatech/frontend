...
export default function ChatClient({
  ...
  return (
    <div className="flex flex-col h-full bg-[#f0f2f5]">
      {/* Messages area — takes available space, scrolls independently */}
      {/* Ad Banner Display */}
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-20">
        <div className="space-y-4">
          {sortedMessages.map((msg) => {
            // FIX: Ensure sender exists before accessing ID
            const isMe = currentUserId !== undefined && msg.sender?.id === currentUserId;

            return (
              <div
                key={msg.id || `temp-${msg.created_at}`} // Use fallback key for optimistic updates
